...
def reconstruct_frames(
    ...
    """
    Express all frames in robot-base coordinates.
    ...
    For sample i:
        ...
        Board through robot chain:
            T_base_ee[i] @ T_ee_board

        Board through camera chain:
            T_base_cam @ T_cam_board[i]

        Camera:
            T_base_cam, constant for every sample
    """
    ...
    for sample_id, (T_base_ee, T_cam_board) in enumerate(
        ...
        board_residual = (
            invert_T(T_base_board_robot)
            @ T_base_board_camera
        )

        translation_error_m = float(
            np.linalg.norm(
                T_base_board_robot[:3, 3]
                - T_base_board_camera[:3, 3]
            )
        )
...
def plot_all_frames(samples, function_name, method_name):
    """
    Plot every sample origin as a point/path, but draw full coordinate frames
    only for the first, middle, and last samples.
    """
    if not samples:
    ...
    T_base_cam = samples[0]["T_base_cam"]
    camera_origin = draw_frame(
        ax, T_base_cam, axis_length=fixed_axis_length,
        label="CAMERA", alpha=1.0, linewidth=3.0,
        draw_axis_labels=True,
    )
    all_points.append(camera_origin)

    ee_origins = np.asarray(
        [sample["T_base_ee"][:3, 3] for sample in samples],
        dtype=np.float64,
    )
    board_robot_origins = np.asarray(
        [sample["T_base_board_robot"][:3, 3] for sample in samples],
        dtype=np.float64,
    )
    board_camera_origins = np.asarray(
        [sample["T_base_board_camera"][:3, 3] for sample in samples],
        dtype=np.float64,
    )
    ...
    ax.plot(
        ...
        linewidth=1.8, alpha=0.85, label="EE path",
    )
    ax.plot(
        board_robot_origins[:, 0], board_robot_origins[:, 1],
        board_robot_origins[:, 2], color=board_robot_path_color,
        marker="o", markersize=5, linewidth=2.0, alpha=0.90,
        label="Board path: robot chain",
    )
    ax.plot(
        board_camera_origins[:, 0], board_camera_origins[:, 1],
        board_camera_origins[:, 2], color=board_camera_path_color,
        marker="x", markersize=6, linewidth=2.0,
        linestyle="dashed", alpha=0.90,
        label="Board path: camera chain",
    )
    ...
    for index in key_indices:
        ...
        T_board_robot = sample["T_base_board_robot"]
        T_board_camera = sample["T_base_board_camera"]

        draw_frame(
            ax, T_base_ee, axis_length=sample_axis_length,
            label=f"EE{sample_id}", alpha=1.0, linewidth=2.2,
        )
        draw_frame(
            ax, T_board_robot, axis_length=sample_axis_length,
            label=f"WR{sample_id}", alpha=1.0, linewidth=2.2,
            linestyle="solid",
        )
        draw_frame(
            ax, T_board_camera, axis_length=sample_axis_length,
            label=f"WC{sample_id}", alpha=1.0, linewidth=2.2,
            linestyle="dashed",
        )

        robot_origin = T_board_robot[:3, 3]
        camera_origin_i = T_board_camera[:3, 3]
        ax.plot(
            [robot_origin[0], camera_origin_i[0]],
            [robot_origin[1], camera_origin_i[1]],
            [robot_origin[2], camera_origin_i[2]],
            color=disagreement_color, linewidth=1.4,
            linestyle="dotted", alpha=0.85,
        )

        ax.scatter(*ee_origins[index], color=ee_path_color, s=55, depthshade=False)
        ax.scatter(*board_robot_origins[index], color=board_robot_path_color, s=55, depthshade=False)
        ax.scatter(*board_camera_origins[index], color=board_camera_path_color, s=55, marker="x", depthshade=False)
        ax.text(
            ee_origins[index, 0], ee_origins[index, 1], ee_origins[index, 2],
            f" S{sample_id}", color=ee_path_color, fontsize=9,
        )
    ...
    displayed_ids = ", ".join(str(samples[index]["sample_id"]) for index in key_indices)
    ax.set_title(
        "Eye-to-Hand Frames in Robot-Base Coordinates\n"
        f"{function_name} — {method_name}\n"
        f"Full frames shown for samples: {displayed_ids}"
    )
...
def main():
    ...
    print("=" * 70)
    print("EYE-TO-HAND CALIBRATION FRAME PLOTTER")
    print("=" * 70)
